...
import axios from '../axios_Inv';  // Axios instance to make API requests
...
const ReportSection3 = () => {
  ...
  const currentUser = { id: savedUsername };
  ...
  const handleDateRangeQuery = async (type) => {
    ...
    try {
      const [mostRes, leastRes] = await Promise.all([
        axios.get(`/items/max-decrease-range/${currentUser.id}?startDate=${startDate}&endDate=${endDate}`),
        axios.get(`/items/min-decrease-range/${currentUser.id}?startDate=${startDate}&endDate=${endDate}`)
      ]);
